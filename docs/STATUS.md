...
- 코지임대 공개 이용약관 현행 정합 (2026-08-28 · 코지 명시 승인 · `app/cozyrent/terms/page.tsx`):
  무료체험 `40일` → **7일**(cozyrent `D-155`) · 체험 중 `베이직 한도` → **수량 제한 없음**(`D-086`) ·
  만료 후 `홈 전용` → **메인 4탭 조회 전용** · 가격 `6,900/12,900/24,900` → **7,700/16,500/25,500**(`D-177`) ·
  플랜 변경을 `D-177`과 정합(상향 = 즉시 + 남은 기간 차액 일할청구 / 하향 = 다음 갱신일부터) ·
  체험 중 결제 시 잔여 체험 소멸(`D-155`) 명시 · 사전신청 30일 무료 이용을 `운영자가 별도로 제공하는
  무료 이용 혜택`으로 7일 체험과 구분(합산 보장 문구 0). README 법적 페이지 설명도 7일로 정합.
  **법률 구조·책임조항 재작성 0. App Store 결제 제공 주장 0.**
  OPEN: 약관 **시행일(2026년 7월 21일) 유지** — 개정 시행일 변경과 사전 고지 필요 여부는 법률 판단이라
  임의로 정하지 않았다. / 체험 중 데이터 반출 제한(cozyrent SUBSCRIPTION §2.3)은 약관에 조항이 없어
  **추가하지 않았다**(이용자 의무 신설은 별도 법률 판단 대상).
