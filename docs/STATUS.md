...
- migration `0014_program_deploy_status.sql` **파일 준비 — 운영 DB 미적용(코지 승인 대기)**.
  적용 전까지 저장 시 신규 필드는 하위호환 재시도로 제외(기존 저장 무영향)
- 승인 후 예정: 0014 실행 → 코지렌트 프로그램을 사전신청 상태(/landingpage/cozyrent · "사전신청하기")로 전환
