...
## Programs Admin 스키마 v2 — Web / Mobile 링크 구조 (2026-06)

Programs 관리자(`/admin/programs`)가 컴퓨터 웹앱과 모바일앱(스토어 배포)을 **공통 구조**로
등록하도록 확장됐다. 모바일앱(예: 감사일기, Gratitude Diary)도 홈페이지 Programs 에 올릴 수 있다.
...
- **Mobile App** — 마이그레이션 `0010_program_mobile_links.sql` 로 다음 컬럼 추가:
  - `play_store_url text` (Google Play)
  - `app_store_url text` (App Store, 선택)
  - `release_status text` — `development` | `coming_soon` | `released` (null=레거시 → released 취급)
- **홈페이지 표시 규칙(상세/카드)**:
  - Mobile `development` → "개발 중" 배지 / `coming_soon` → "출시 예정" 배지(둘 다 비활성).
  - Mobile `released` → 등록된 스토어 URL만 버튼으로("Google Play에서 받기" / "App Store에서 받기").
    App Store URL이 비어 있어도 오류 없이 Google Play 버튼만 노출된다.
- **감사일기 등록 형태**: `type=mobile`, `release_status=released`, `play_store_url=Google Play 링크`,
  `app_store_url`은 빈 값(향후 iOS 출시 시 추가 가능).
- **하위 호환**: 기존 `app_url` 기반 웹 프로그램 실행 버튼은 그대로 동작한다(스키마/코드 변경 없음).
  `0010` 미실행 환경에서도 `saveProgram` 이 모바일 컬럼 부재 오류를 감지해 제외 후 재시도한다.
